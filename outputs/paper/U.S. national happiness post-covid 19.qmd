...
To get better insights on what factors affect happiness, we utilized the 2021 US General Social Survey [@norc] from NORC. The raw dataset is extracted from the survey and was imported in R using the package Haven [@haven]. It includes data from 568 survey questions for 4,032 survey participants. Using the R [@citeR] package tidyverse [@tidyverse] and dplyr [@dplyr], we were able to clean and perform exploratory data analysis on the dataset to get insights into the data. Further, we used R package ggplot2 [@ggplot2], Knitr [@knitr] and KableExtra [@kbextra] to visualize the data for this paper. The dataset used to explore the overall understanding of happiness in age, sex, and racial groups, as well as the relationship between working conditions and happiness was directly extracted from the GSS Data Explorer website (<https://gssdataexplorer.norc.org/home>). All the interested variables were first selected from the website and then added to an extract, which was later exported in the Excel Workbook format. The R package readxl [@readxl] was used to read the Excel Workbook file in R.
