...
sum_var_job <- cleaned_data_job %>% 
  select(hrs1, rincome)

# summary table age
sumtable(sum_var, title = "Summary table of age variable", out = 'kable', simple.kable = TRUE)
```

```{R}
sumtable(sum_var_job, title = "A", out = "kable", simple.kable = TRUE)
...
The low response rate (17.4%) also heavily affects the final results, that it cannot generalize and reflect the whole population.
...
>>>>>>> 30a638ecf3c25c2d07bad4b2cd4d6e5250b61738
```
